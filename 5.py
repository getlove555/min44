...
ki = ASUL.LINE()
ki.login(token='EoUjmmGwyMSDni44oP27.MRMd87JLMY8NA0SCe7JEXW.li3gJGcnFqv82RTJ5v613jS3R6WFcKAOU+O2JbDM5Ds')
ki.loginResult()
...
ki3 = ASUL.LINE()
ki3.login(token='EoDoya8Kzg4iEofYLL5f.+N7cEp6EA/PZBKGTFUuatW.7DJ3nRrSYECQAwczQBXBhtf5L/O9dPYU/+RMD9jny14')
ki3.loginResult()
...
#-----------------------------------------------------------
                ...
                if msg.toType == 2:
                    ...
                    if matched_list == []:
                        ...
                        ki2.sendText(msg.to,"Daftar hitam pengguna tidak memiliki")
                        ki3.sendText(msg.to,"Daftar hitam pengguna tidak memiliki")
                        ki4.sendText(msg.to,"Daftar hitam pengguna tidak memiliki")
                        ki5.sendText(msg.to,"Daftar hitam pengguna tidak memiliki")
                        ki6.sendText(msg.to,"Daftar hitam pengguna tidak memiliki")
#----------------------------------------------------------------------------
#------------------------------- COVER BY TAG -------------------------------
...
#--------------------------------------------------------
            ...
            elif msg.text in ["Allprotect on","ล้อกเปิด"]:
              if msg.from_ in admin:
                if wait["inviteprotect"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Ini sudah on 􀜁􀇔􏿿👈")
                ...
                if wait["linkprotect"] == True:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"Already on")
            elif msg.text in ["Allprotect off","ล้อกปิด"]:
              if msg.from_ in admin:
...
#-----------------------------------------------------------
              ...
              if msg.from_ in admin:
                ...
                cl.findAndAddContactsByMid(midd)
                cl.inviteIntoGroup(msg.to,[midd])
            elif msg.text in ["k1 Invite:","ดึง:"]:
              if msg.from_ in admin:
                midd = msg.text.replace("Invite: ","")
                ki.findAndAddContactsByMid(midd)
                ki.inviteIntoGroup(msg.to,[midd])
            elif msg.text in ["k2 Invite:","ดึง:"]:
              if msg.from_ in admin:
                midd = msg.text.replace("Invite: ","")
...
#----------------------------------------------------------
                ...
                try:
                    ...
                    cl.sendText (msg.to, "Can not find what you want to find (` · ω · ') ")
            elif 'Song' in msg.text:
                try:
                    textToSearch = (msg.text) .replace ('music request', "") .strip ()
                    query = urllib.quote (textToSearch)
                    url = "https://www.youtube.com/results?search_query=" + query
                    response = urllib2.urlopen (url)
                    html = response.read ()
