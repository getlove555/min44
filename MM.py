...
-----------------------
🔥||✒️✒ Bot:ct 
       - Broadcast to All Contact BOT
-----------------------
🔥||✒️✒ Bot:grup  
    - Broadcast to All Grup Joined BOT
-----------------------
🔥||✒️✒ Allname:  
    - Change All Name BOT
-----------------------
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"selt bot by\n\nมินบอทอินดี้\n\nที่แอดมามีอะไรหรือป่าวครับส่งขอความไว้\nแล้วจะกลับมาอ่านครับ",
    "lang":"JP",
...
# ----------------- NOTIFED MEMBER OUT GROUP
        if op.type == 15:
            if op.param2 in bot1:
                return
            cl.sendText(op.param1,"ปาร์ตี้ ย่อมมีการ เลิกลา \n\nแล้วพบกันใหม" + cl.getContact(op.param2).displayName + " ลาก่อย\n\nแล้วเจอกัน\nเมื่อชาติต้องการ\nBY.มินบอทอินดี้")
            print ("MEMBER HAS LEFT THE GROUP")
#------------------ KICK OUT FORM GROUP
        if op.type == 19:
            if op.param2 in bot1:
                return
            cl.sendText(op.param1,cl.getContact(op.param2).displayName + " โอโหจังหวะนี้ซัดเต็มข้อเลยครับ ท่านผู้ชม")
...
# ----------------- NOTIFED MEMBER JOIN GROUP
        if op.type == 17:
            ...
            cl.sendText(op.param1, "ยินดีต้อนรับเข้าสู้กลุ่มขอเรานะครับ " + cl.getContact(op.param2).displayName + "กลุ่มนี้ยินดีต้อนรับครับ " + "👉" + str(ginfo.name) + "👈""\n\nมาแล้วก็อย่าสร้างความวุ่นวายนะครับ \nไม่งั้นผมจะถีบส่งให้")
...
#----------------------ADMIN COMMAND------------------------------#
              ...
              if msg.from_ in owner:
                start = time.time()
                cl.sendText(msg.to, "อะไรยังไงหว่าา...")
                elapsed_time = time.time() - start
                cl.sendText(msg.to, "%เห้อออ" % (elapsed_time))
